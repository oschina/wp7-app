...
//WP7_ControlsLib.Controls.ProgressTrayPage
...
namespace OSChina
{
    public static class Tool
    {
        ...
        public static string HtmlProcess_Post(PostDetail p)
        {
            ...
            string html = string. Format( "<body style='background-color:#FFFFFF;'>{0}<div id='oschina_title'>{1}</div><div id='oschina_outline'>{2}</div><hr/><div id='oschina_body'>{3}</div>{4}</body>", Config. HTML_Style, p. title, author_str, p. body, Config. HTML_Bottom );
            html = ControlHelper. ConvertExtendedASCII( html );
            return html;
        }
        ...
        {
            //string pubTime = string. Format( "在{0} 更新了动态 <br/>{1}", Tool. IntervalSinceNow( t. pubDate ), GetAppClientString( t. appClient ) );
